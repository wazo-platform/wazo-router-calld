...
def update_ipbx(db: Session, ipbx_id: int, ipbx: schema.IPBXUpdate):
    ...
    if db_ipbx is not None:
        db_ipbx.tenant_id = ipbx.tenant_id if ipbx.tenant_id is not None else db_ipbx.tenant_id
        db_ipbx.domain_id = ipbx.domain_id if ipbx.domain_id is not None else db_ipbx.domain_id
        db_ipbx.customer = ipbx.customer if ipbx.customer is not None else db_ipbx.customer
        ...
        db_ipbx.registered = ipbx.registered if ipbx.registered is not None else db_ipbx.registered
        db_ipbx.username = ipbx.username if ipbx.username is not None else db_ipbx.username
